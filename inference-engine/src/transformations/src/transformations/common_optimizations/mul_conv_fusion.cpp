...
//NGRAPH_RTTI_DEFINITION(ngraph::pass::MultiplyConvolutionFusion<ngraph::opset8::Convolution>, "MultiplyConvolutionFusion", 0);

template <typename T>
ngraph::pass::MultiplyConvolutionFusion<T>::MultiplyConvolutionFusion() {
    ...
    auto conv_pattern = ngraph::pattern::wrap_type<T>({mul_pattern, weights_pattern});
    ...
    matcher_pass_callback callback = [=](pattern::Matcher & m) -> bool {
        ...
        if (op::util::check_for_broadcast(weights.get_shape(), mul_const.get_shape())) {
            return false;
        }
