...
static bool has_dequantization_subgraph(const std::shared_ptr<ov::Node>& first_convert) {
    ...
    const auto subtract = std::find_if(convert_or_subtract_users.begin(),
                                       ...
                                       [](const std::shared_ptr<ov::Node>& n) -> bool {
                                           return ov::is_type<ov::op::v1::Subtract>(n);
                                       });
...
ov::pass::CompressQuantizeWeights::CompressQuantizeWeights() {
    ...
    auto weigths_convert_pattern = pattern::wrap_type<op::v0::Convert>({weights_const_pattern});
    ...
    ov::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        if (fq_users.size() == 1 && has_dequantization_subgraph(fq_users[0])) {
            ...
            if (auto new_weights = ov::get_constant_from_source(first_convert)) {
                ...
                if (weights_users.size() == 1 && ov::is_type<ov::op::v0::Convert>(weights_users[0])) {
                    disable_constant_folding(weights_users[0]);
                }
                return true;
            } else {
                return false;
            ...
            */
            ...
            std::shared_ptr<Node> new_output_high =
                std::make_shared<op::v1::Add>(new_output_low,
            ...
            std::shared_ptr<Node> new_weights = std::make_shared<op::v0::Convert>(quantize, quantized_type);
            ...
            */
            ...
            if (should_convert) {
                input_low = std::make_shared<op::v0::Convert>(input_low, element::f32);
                input_high = std::make_shared<op::v0::Convert>(input_high, element::f32);
                output_low = std::make_shared<op::v0::Convert>(output_low, element::f32);
                output_high = std::make_shared<op::v0::Convert>(output_high, element::f32);
                new_output_low = std::make_shared<op::v0::Convert>(new_output_low, element::f32);
                new_output_high = std::make_shared<op::v0::Convert>(new_output_high, element::f32);
            }
            auto output_range = std::make_shared<op::v1::Subtract>(output_high, output_low);
            auto input_range = std::make_shared<op::v1::Subtract>(new_output_high, new_output_low);
            std::shared_ptr<Node> scale = std::make_shared<op::v1::Divide>(output_range, input_range);
            auto descaled_output_low = std::make_shared<op::v1::Divide>(output_low, scale);
            std::shared_ptr<Node> shift = std::make_shared<op::v1::Subtract>(new_output_low, descaled_output_low);
            OPENVINO_SUPPRESS_DEPRECATED_START
            if (auto constant = ov::get_constant_from_source(scale)) {
                OPENVINO_SUPPRESS_DEPRECATED_END
                scale = constant;
            }
            auto zero = op::v0::Constant::create(scale->get_output_element_type(0), Shape{}, {0});
            auto scale_eq_zero = std::make_shared<op::v1::Equal>(scale, zero);
            // shift equals to input_low - output_low / scale
            // for positions where scale == 0, we put zero as shift
            std::shared_ptr<Node> zero_point = std::make_shared<op::v1::Select>(scale_eq_zero, zero, shift);

            if (should_convert) {
                scale = std::make_shared<op::v0::Convert>(scale, fq_type);
                zero_point = std::make_shared<op::v0::Convert>(zero_point, fq_type);
            }
            ...
            auto convert_to_high_prec = std::make_shared<op::v0::Convert>(new_weights, input_type);
            auto sub = register_new_node<op::v1::Subtract>(convert_to_high_prec, zero_point);
            auto mul = register_new_node<op::v1::Multiply>(sub, scale);
            mul->set_friendly_name(fq->get_friendly_name());
            copy_runtime_info(fq, {convert_to_high_prec, sub, mul});
            disable_constant_folding(convert_to_high_prec);
            replace_node(fq, mul);
        }
        return true;
    };

    auto m = std::make_shared<pattern::Matcher>(fq_pattern, "CompressQuantizeWeights");
    this->register_matcher(m, callback);
}

ov::pass::ZeroPointOptimizer::ZeroPointOptimizer() {
    auto weights_pattern = pattern::wrap_type<op::v0::Constant>();
    auto zero_point_pattern = pattern::wrap_type<op::v0::Constant>();
    auto convert_pattern = pattern::wrap_type<op::v0::Convert>({weights_pattern});
    auto sub_pattern = pattern::wrap_type<op::v1::Subtract>({convert_pattern, zero_point_pattern});
    ...
    ov::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        auto weights =
            std::dynamic_pointer_cast<op::v0::Constant>(pattern_value_map.at(weights_pattern).get_node_shared_ptr());
        if (!weights || weights->get_element_type() != element::i8)
            return false;
        auto zero_point =
            std::dynamic_pointer_cast<op::v0::Constant>(pattern_value_map.at(zero_point_pattern).get_node_shared_ptr());
        if (!zero_point)
            return false;
        ...
        auto int8_zero_point = std::make_shared<op::v0::Convert>(
            std::make_shared<op::v5::Round>(zero_point, op::v5::Round::RoundMode::HALF_TO_EVEN),
            weights->get_element_type());
        auto adj_zero_point = std::make_shared<op::v1::Subtract>(
            zero_point,
            std::make_shared<op::v0::Convert>(int8_zero_point, convert->get_element_type()));
        ...
        auto transformed = std::make_shared<op::v1::Subtract>(
            std::make_shared<op::v0::Convert>(std::make_shared<op::v1::Subtract>(weights, int8_zero_point),
                                              convert->get_element_type()),
            adj_zero_point);
        auto diff = std::make_shared<op::v1::Subtract>(sub, transformed);
        OPENVINO_SUPPRESS_DEPRECATED_START
        ...
        std::shared_ptr<Node> new_weights = std::make_shared<op::v1::Subtract>(weights, int8_zero_point);
        OPENVINO_SUPPRESS_DEPRECATED_START
        if (auto constant = ov::get_constant_from_source(new_weights)) {
            OPENVINO_SUPPRESS_DEPRECATED_END
            new_weights = constant;
    ...
    auto m = std::make_shared<pattern::Matcher>(sub_pattern, "ZeroPointOptimizer");
    this->register_matcher(m, callback);
}
