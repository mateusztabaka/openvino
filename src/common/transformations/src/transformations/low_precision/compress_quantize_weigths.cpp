...
ngraph::pass::CompressQuantizeWeights::CompressQuantizeWeights() {
    ...
    ngraph::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        if (fq_users.size() == 1 && has_dequantization_subgraph(fq_users[0])) {
            ...
            if (auto new_weights = ov::constantfold_subgraph(first_convert)) {
            ...
            */
            ...
            if (auto constant = ov::constantfold_subgraph(scale))
                scale = constant;
            ...
            if (auto constant = ov::constantfold_subgraph(zero_point))
                zero_point = constant;
            if (auto constant = ov::constantfold_subgraph(scale))
                scale = constant;
...
ngraph::pass::ZeroPointOptimizer::ZeroPointOptimizer() {
    MATCHER_SCOPE(ZeroPointOptimizer);
    auto weights_pattern = pattern::wrap_type<opset8::Constant>();
