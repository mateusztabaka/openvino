...
bool ov::pass::UselessSliceEraser::run_on_model(const std::shared_ptr<ngraph::Function>& f) {
    ...
    for (auto& node : f->get_ordered_ops()) {
        ...
        if (!is_slice || node->get_output_partial_shape(0).is_dynamic() || node->get_input_partial_shape(0).is_dynamic())
            continue;
        if (node->input(0).get_shape() != node->output(0).get_shape())
            continue;

        auto stridesNode = std::dynamic_pointer_cast<ov::op::v0::Constant>(node->input_value(3).get_node_shared_ptr());
        if (stridesNode) {
