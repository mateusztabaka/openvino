...
void jit_dft_kernel_f32<isa>::generate() {
    ...
    Xmm xmm_twidles = Xbyak::Xmm(reg_idx++);
    ...
    auto nonsimd_loop = [&] {
        ...
        auto c2r_kernel = [&] (bool backwards) {
            ...
            uni_vmovq(xmm_twidles, ptr[twiddles_ptr]);
            uni_vmulps(xmm_input, xmm_input, xmm_twidles);
            if (!backwards) {
        ...
        auto c2c_kernel = [&] (bool backwards) {
            ...
            uni_vmovq(xmm_twidles, ptr[twiddles_ptr]);
            shufps(xmm_twidles, xmm_twidles, 0b01000100);
            xorps(xmm_twidles, neg_mask);
            uni_vmulps(xmm_input, xmm_input, xmm_twidles);
            if (!backwards) {
